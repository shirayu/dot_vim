...
if [[ $1 == "load" ]]; then
    ...
    xargs -t pip install < "$HOME/.vim/locl/pip.lock" 

elif [[ $1 == "update" ]]; then
    vim +':call dein#update()' +q
    vim +':call dein#save_rollback("$HOME/.vim/lock/dein.lock.json.tmp")' +q
    python -m json.tool <"$HOME/.vim/locl/dein.lock.json.tmp" >"$HOME/.vim/lock/dein.lock.json"
    rm "$HOME/.vim/locl/dein.lock.json.tmp"

    pip install -U isort black
    pip list | grep -e 'isort' -e 'black' | sed -r 's/\s+/==/' > "$HOME/.vim/locl/pip.lock"
